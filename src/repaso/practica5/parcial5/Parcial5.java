...
public class Parcial5 {
    ...
    List<String> estadios(Graph<Estadio> mapaEstadios, String estadioOrigen, int cantKm){
        ...
        if (mapaEstadios != null && !mapaEstadios.isEmpty()){
            ...
            if (origen != null){
                ...
                contarEstadios(mapaEstadios, origen, cantKm, visitados, lista, new LinkedList<String>());
            }
        }
        return lista;
    }

    private void contarEstadios(Graph<Estadio> mapaEstadios, Vertex<Estadio> origen, int cantKm, boolean [] visitados, List<String> listaFinal, LinkedList<String> listaActual){
        visitados[origen.getPosition()] = true;

        if (cantKm >= 0){
            listaActual.add(origen.getData().getEstadio());
            List<Edge<Estadio>> edges = mapaEstadios.getEdges(origen);
            for (Edge<Estadio> arista : edges){
                Vertex<Estadio> destino = arista.getTarget();
                if (!visitados[destino.getPosition()])
                    contarEstadios(mapaEstadios, destino, (cantKm - arista.getWeight()) , visitados, listaFinal, listaActual);
            }
        } else {
        ...
        if (cantKm >= 0)
            listaActual.removeLast(); // si agregue, borro
