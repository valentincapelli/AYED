...
public class VisitaOslo {
	...
	private boolean busquedaRecursiva(Graph<String> lugares, Vertex<String> origen, String destino, int maxTiempo, List<String> lugaresRestringidos, boolean [] visitados, int tiempo, List<String>camino){
		...
		if (origen.getData() == destino)
			...
			while (it.hasNext() && !cumple){
				...
				if (!visitados[dest.getTarget().getPosition()] && maxTiempo >= tiempo && !lugaresRestringidos.contains(dest.getTarget().getData())){
					cumple = busquedaRecursiva(lugares, dest.getTarget(), destino, maxTiempo, lugaresRestringidos, visitados, (tiempo+dest.getWeight()), camino);
				}
			}
		}
		if (!cumple)
